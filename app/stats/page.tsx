        ...
        {TABS.map((t) => (
          <Link
            ...
            className={`rounded px-3 py-1.5 text-sm font-medium ${
              activeTab === t.key ? "bg-felt text-white" : "border border-ink/15 text-ink/70"
            }`}
          >
            {t.label}
          </Link>
...
function RecordTable({
  ...
  if (rows.length === 0) {
    return <p className="text-ink/50">No frames recorded yet.</p>;
  }
  ...
  return (
    <div className="overflow-x-auto rounded-lg border border-ink/10">
      <table className="w-full min-w-[560px] border-collapse text-sm">
        ...
        <tbody>
          {rows.map((row, i) => (
            <tr key={row.player_id} className="border-b border-ink/5">
              <td className="py-2 pr-4 text-ink/50">{i + 1}</td>
              <td className="py-2 pr-4 font-medium">
                {playersById.get(row.player_id)?.name ?? "Unknown"}
              </td>
              <td className="py-2 pr-4 text-ink/50">{teamNameFor(row.player_id)}</td>
...
function BreaksTable({
  ...
  if (rows.length === 0) {
    return <p className="text-ink/50">No break wins recorded yet.</p>;
  }
